...
<div id="change_popup" style="width: 600px;margin: 0 auto;background: #FFF;padding: 15px;min-height: 600px;" class="popup-basic admin-form mfp-with-anim mfp-hide" >
    <form id="changeFrm" method="post" action="<?php echo Yii::app()->request->baseUrl; ?>/supplier/inteditchaque"></form>
</div>
                            ...
                            <?php if (count($model)): ?>
                                ...
                                <tbody>
                                    ...
                                    foreach ($model as $posValue):
                                        $customer_id = $posValue['supplier_id'];
                                        $q1 = new CDbCriteria(array('condition' => "id = '$customer_id'",));
                                        $customers = Supplier::model()->findAll($q1);
                                        if (count($customers)): foreach ($customers as $customer): $customer_name = $customer->name;
                                            endforeach;
                                        else: $customer_name = "";
                                        endif;
                                        ...
                                        <tr style="font:Verdana; font-size:11px">
                                            ...
                                            <td class="change_payment" data-id="<?php  echo $posValue['id']; ?>" style="background: <?php echo $posValue['is_confirm'] != "0"?"#1c94c4":"#900000"; ?>;color: #FFFFFF;cursor: pointer;"><?php echo $posValue['is_confirm'] != "0"?"Paid":"Returned"; ?>  </td>
                                        </tr>
                                        <?php
                                        $i = $i + 1;
                                    ...
                                    ?>
                                    <tr>
                                        <td colspan="3"><strong><?php echo "Total"; ?></strong><strong style="float: right;"><?php echo "&pound; " . number_format($grand_total, 2); ?></strong></td>
                                         
                                        </tr>
                                <?php else: ?>
                                    <tr><div id="message-red"><td colspan="5" class="red-left">No Received Amount Available Yet .. <a href="<?php echo Yii::app()->request->baseUrl . '/supplier/cheque_payment/'; ?>">Search Again..</a></td></div></tr>
                                <?php endif; ?>
                        </table>
                    </div>
                </div>
