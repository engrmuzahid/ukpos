...
<script type="text/javascript">
	function getCustomer()
	{
		...
		xm.send(null);
		msg=xm.responseText;	
		var mySplitResult = msg.split("_");
		document.getElementById("customer").innerHTML = mySplitResult[0];	
		document.getElementById("comment").value = mySplitResult[1];				
	}
	...
	function getDiscountValue2()
	{
		...
		function MyCart2()
		{
			...
			oForm.action="<?php echo Yii::app()->request->baseUrl.'/b2b_sell/add'; ?>";
			oForm.post="post";
			oForm.submit();
		}
		...
		{
			var oForm = document.frm_soft;
			oForm.action="<?php echo Yii::app()->request->baseUrl.'/b2b_sell/suspend'; ?>";
			oForm.post="post";
			oForm.submit();
		}
		
		function MySellCancel2()
		{
			var oForm = document.frm_soft;
			oForm.action="<?php echo Yii::app()->request->baseUrl.'/b2b_sell/sell_cancel'; ?>";
			oForm.post="post";
			oForm.submit();
		}
		...
		{
			var oForm = document.frm_soft;
			oForm.action="<?php echo Yii::app()->request->baseUrl.'/customer_invoice'; ?>";
			oForm.post="post";
			oForm.submit();
		}
	...
	function getBalance()
	{
		...
		document.getElementById("cash_balance").value = amount_balance;				
            ...
            <?php $this->renderPartial('_entryform', array('model' => $model, 'customer_id2' => $customer_id, 'old_order'=>isset($old_order) ? $old_order : array() ))?>
