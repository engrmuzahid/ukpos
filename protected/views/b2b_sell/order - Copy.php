...
<script type="text/javascript">
    ...
    var eu_enable = '<?php echo $company['eu'] ?>';
    var customer_enable = '<?php echo $company['customer_mandatory'] ?>';
        ...
        $.each(cart, function (k, val) {
            if (val) {
                ...
                if (product) {
                    ...
                    content += '<td><input readonly data-id="' + product.product_code + '" type="text" size="1" class="cart_qty" value="' + val.quantity + '"></td><td><input  readonly data-id="' + product.product_code + '" class="product_name" style="width:98%;' + _style + '" type="text" value="' + pname + '"><div class="lastbuy">' + lastbuy + '</div></td><td>' + (parseFloat(stock_amt)).toFixed(0) + '</td>';
                    content += (eu_enable == '1') ? '<td style="color:silver" data-id="' + product.product_code + '"  class="show_last_history"><span class="tooltiptext" id="tooltiptext_' + product.product_code + '">Loading...</span>' + parseFloat(parseFloat(product.purchase_cost) + parseFloat(product.vat_on_purchase)).toFixed(2) + '</td>' : '';
                    ...
                    content += '<td><input readonly data-id="' + val.product_code + '" type="text" size="1" class="cart_qty" value="' + val.quantity + '"></td><td><input readonly data-id="' + val.product_code + '" class="product_name" style="width:98%;' + _style + '" type="text" value="' + val.product_code + '"></td><td></td><td style="color:silver"></td><td><input  readonly style="' + _style + '" type="text" class="product_price" size="1" data-id="' + val.product_code + '" value="' + pprice + '"></td>';
                    content += '<td>' + price + '</td>';
                    content += '<td><a href="javascript:void(0)" class="cart_add" data-id="' + val.product_code + '"><img src="' + base_url + '/public/images/icon_add.png"></a> <a href="javascript:void(0)" class="cart_remove" data-id="' + val.product_code + '"><img src="' + base_url + '/public/images/delete.png"></a></td></tr>';
                    arr_content[cnt_key++] = content;
                    item_count++;
...
<?php endif; ?>
        ...
        $("#search_product").live("keyup", function e() {
            // check_invoice_data($(this).val());            
            $.post(base_url + '/public/product_list_for_client.php?customer_id=' + $("#customer_id").val(), {'product': $(this).val()}, function (resp) {
...
//        
        ...
        $("#print_order").click(function (e) {
            ...
            var url = base_url + '/b2b_sell/addOrder?b2b=<?php echo $company['printer_name']; ?>';
            $("#cart_info").val(JSON.stringify(cart));
            if (check_input()) {
                if ($("#customer_id").val() == 0) {
                    url = base_url + '/b2b_sell/addOrderWithoutCustomer?b2b=<?php echo $company['printer_name']; ?>';
                }
                setCookie('pos_cart', "", 1);
    ...
    <div class="scroller" id="cart_section">        
        <div id="total_price">
            ...
            <div style="float: left; margin-top: 10px; font-size: 30px; width: 40%; font-weight: bold; text-align: center">
                ...
                <div style="font-size: 13px; margin-top: 5px; clear: both;display: none;">Vat: &pound;<span id="vat_amount">0.00</span></div>
                <div style="font-size: 13px; margin-top: 5px; clear: both">Disc: &pound;<span id="discount_amount">0.00</span></div>                
            </div>
            <div style="float: right; width: 30%">
                <?php if ($company['is_suspend'] == '1'): ?>
        ...
        <div id="cart_header" style="padding: 5px 0">
            ...
            <table style="float: right; width: 70%;">
                <?php if ($company['payment_check'] == '1'): ?>
                    <tr>
            ...
            <div>
                ...
                <span id="customer"></span><span id="buy_price" style="font-size: 11px; font-weight: normal; color: #BBB;display:none;"></span>
            </div>
