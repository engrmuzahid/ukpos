<style type="text/css">    
    ...
    .producthistorydiv .producthistorylistdiv {
        visibility: hidden;
        background-color: black;
        color: #fff;
        text-align: center;
    ...
    .producthistorydiv:hover .producthistorylistdiv {
        visibility: visible;
    }
</style>
...
<script type="text/javascript">
    ...
    var json_categories = <?php echo $categories ?>;
    var json_products = <?php echo $products ?>;
    var json_stocks = <?php echo $stocks ?>;
    var json_customers = {};
    var json_temp_products = <?php echo $temp_products ?>;
    var base_url = '<?php echo Yii::app()->baseUrl ?>';
    var eu_enable = '<?php echo $user['eu'] ?>';
    var customer_enable = '<?php echo $user['customer_mandatory'] ?>';
    ...
    function list_products(_products) {
        ...
        if (_products) {
            ...
            $.each(_products, function (k, product) {
                if (product) {
                    var p_price_new = parseFloat(product.wholesale_price) > 0 ? product.wholesale_price : product.sell_price;

                    content = '<div class="product_item" style="background-color:' + color_code + '"  data-name="' + product.product_name + '" data-price="' + p_price_new + '" data-id="' + product.product_code + '" id="product_item_' + product.product_code + '"><img  alt="' + product.product_name + '" src="' + base_url + '/images/products/' + product.image + '" /></div>';
    ...
    function list_invoice(_products) {
        ...
        if (_products) {
            ...
            $.each(_products, function (k, product) {
                if (product) {
                    var p_price_new = parseFloat(product.wholesale_price) > 0 ? product.wholesale_price : product.sell_price;
                    content = '<div class="product_item" data-name="' + product.product_name + '" data-price="' + p_price_new + '" data-id="' + product.product_code + '" id="product_item_' + product.product_code + '"><img  alt="' + product.product_name + '" src="' + base_url + '/images/products/' + product.image + '" /><br>&pound;' + parseFloat(product.sell_price).toFixed(2) + '</div>';
                    $("#product_list").append(content);
                }
            });
        } else {
                ...
                if (product) {
                    ...
                    if (parseFloat(val.product_price) > 0) {
                        ...
                        if (product.wholesale_price > 0) {
                            var pprice = parseFloat(product.wholesale_price);
                        } else {
                    ...
                    item_count++;
                    b_price = parseFloat(b_price) + ((parseFloat(product.purchase_cost)+  parseFloat(product.vat_on_purchase)) * parseFloat(val.quantity));
                  //  alert(product.vat_on_purchase);
					...
					console.log("Sell price: " + pprice);
					console.log("Vat on profit: " + profitVat);
    ...
    function calculate_amount() {
        ...
        // alert("main total "+ new_amount +"discount  "+ discount_amount);
    ...
    function printOrder() {
        ...
        if (check_input()) {
            if ($("#customer_id").val() == 0) {
                url = base_url + '/b2b_sell/addOrderWithoutCustomer?b2b=<?php echo $user['printer_name']; ?>';
            }
            setCookie('pos_cart', "", 1);
    ...
    var discount_total = '<?php echo @$_GET['discount'] ? $_GET['discount'] : 0; ?>';
...
<?php if (@$_GET['customer_id']): ?>
            setCookie('pos_customer', <?php echo $_GET['customer_id']; ?>);
<?php endif; ?>

        if (parseFloat(discount_total) > 0) {
            $("#discount").val(parseFloat(discount_total));
            $("#discount_amount").html(parseFloat(discount_total));
            $("#discount_type").val("flat");
        }


        ccustomer = getCookie("pos_customer");
        if (ccustomer > 0) {
...
//
//        $(".product_list_view").live('mouseenter', function (e) {
//            var pcode = $(this).attr('data-id');
//
//            $('#tooltiptext_' + pcode).css('visibility', 'visible');
//            var url = '<?php echo Yii::app()->request->baseUrl . '/b2b_sell/show_product_history'; ?>';
//            $.post(url, {'pcode': pcode}, function (resp) {
//                $('#product_tooltiptext_' + pcode).html(resp);
//            });
//        });
//
//        $(".product_list_view").live('mouseleave', function (e) {
//            var pcode = $(this).attr('data-id');
//            $('#product_tooltiptext_' + pcode).css('visibility', 'hidden');
//        });
        ...
        $("#print_order").click(function (e) {
            ...
            if ($("#customer_id").val() != 0) {
                ...
                if ((amount + total_due) > credit_limit) {
                    ...
                    setTimeout(function () {
                        document.getElementById('approvalpassfield').focus();
                    }, 100);
                } else {
                    printOrder();
                }
        ...
        $("#submitpasswrd").click(function (e) {
            e.preventDefault();
            var pass = $("#approvalpassfield").val();
            if (pass != null && pass != "<?php echo $company['passcode']; ?>") {
                $("#passworerror").html("Please enter correct code");
            } else {
                printOrder();
                $.magnificPopup.close();
        ...
        $(".customer_radiobox").live('click', function () {
            ...
            $.post(url, {}, function (resp) {
                ...
                $("#customer").append('&nbsp;<input type="button" class="buttonGreen" id="customer_invoice" value="Inv." />');
                $("#product_list").empty();
                console.log($("#customer_due_amount").val());
                console.log($("#credit_limit").val());

                setTimeout(function () {
                    document.getElementById('search_product').focus();
                }, 100);
                if (parseFloat($("#customer_due_amount").val()) > 0 && parseFloat($("#customer_due_amount").val()) > parseFloat($("#credit_limit").val())) {
        ...
        $(".product_list_view").live('mouseenter', function (e) {
            var pcode = $(this).attr('data-id');
            var custid = $(this).attr('data-customer');

            $('#product_tooltiptext_' + pcode).css("visibility", "visible");
            var url = '<?php echo Yii::app()->request->baseUrl . '/b2b_sell/show_product_history'; ?>';
            $.post(url, {'pcode': pcode, 'custid': custid}, function (resp) {
                $('#product_tooltiptext_' + pcode).html(resp);
            });
        });
...
<div style="width:700px;margin: 0 auto;background: #FFF;padding: 15px;min-height: 600px;" class="popup-basic admin-form mfp-with-anim mfp-hide" id="customer_popup">
    <form method="post" id="search_frm" action="<?php echo Yii::app()->baseUrl ?>/b2b_sell/addOrder">
        <div style="width: 100%;height: 50px;padding-top: 15px;">
            <input type="text" class="search_customer" id="search_customer_name" autofocus="true"  name="Search[customer_name]" style="height: 35px; width: 200px;margin-right: 10px;padding-left: 10px;float: left;" placeholder="CUSTOMER NAME"> 
            <input type="text" class="search_customer" name="Search[contact_no]" style="height: 35px; width: 200px;margin-right: 10px;padding-left: 10px;float: left;" placeholder="PHONE"> 
            <input type="text" class="search_customer" name="Search[postcode]"  style="height: 35px; width: 200px;margin-right: 10px;padding-left: 10px;float: left;" placeholder="POSTCODE"> 
        </div>
    </form>
    <div id="customer_listing">
            ...
            foreach ($org_customers as $customer):
                ...
                <tr>
                    <td style="line-height: 27px;cursor: pointer;" class="customer_radiobox" data-name="<?php echo "{$customer['customer_name']}, {$customer['business_name']}"; ?>"  data-value="<?php echo $customer['id']; ?>" >
                        <b><?php echo "{$customer['customer_name']}, {$customer['business_name']}" ?>   </b>  <br/>
...
<form method="post" id="frm_order" action="<?php echo Yii::app()->baseUrl ?>/b2b_sell/addOrder">
    ...
    <div class="scroller" id="cart_section">        
        ...
        <div id="cart_header" style="padding: 5px 0;min-height: <?php echo $user['customer_mandatory'] == '1' ? "30px" : "100px"; ?>">
            <table style="float: left; width: 30%; text-align: left;display: <?php echo $user['customer_mandatory'] == '1' ? "none" : ""; ?>">
                <tr>
            ...
            <table style="float: right; width: 70%;display: <?php echo $user['customer_mandatory'] == '1' ? "none" : ""; ?>">
            ...
            <div style="clear: both"></div>
            <div style="border-top: 1px dashed; width: 100%; margin: 10px 0;display: <?php echo $user['customer_mandatory'] == '1' ? "none" : ""; ?>"></div>            
            <div>
                <input type="button" style="float: left;" id="customer_order" class="buttonGreen" value=" CUSTOMER " />
                <input type="text" style="width: 30%;margin-left: 10px;" id="comment" name="comment" />
                <span id="customer_typeEE" style="display:none;"></span>
        ...
        <div id="cart_table">
            <table id="register" style="width: 100%">
                ...
                <tbody class="scroller" id="cart_items"></tbody>
            </table>
        </div>
    </div>
    <div id="item_listing">
...
<div id="approvalpass" style="padding: 50px;background: white;" class="popup-basic admin-form mfp-with-anim mfp-hide" >
    <p>Credit limit exceeded.Enter approval code</p><br/>
    <input type="password" id="approvalpassfield" style="width: 100%;padding: 10px;" placeholder="Password"/><br/>
    <span id="passworerror" style="color:red;"></span>
    <br/>
    <input type="button" style="float: right;" id="submitpasswrd" class="buttonGreen" value=" Submit " />
</div>
