    ...
    public function actionReportListing() {
        if (count($_POST)) {
            ...
            $this->renderPartial('sell_report', array(
                ...
                'customer_id_' => $_POST['customer_id'] != "" ? $_POST['customer_id'] : 0,
...
//            exit();
            $mail = Yiimail();
            $mail->addAddress($Customer[0]['email_address'],$Customer[0]['customer_name']);
         //   $mail->addAddress("muzahid_cse@live.com", $Customer['customer_name']);
            $mail->Subject = 'Account Invoice';


            $mail->msgHTML("<h3>Dear Customer,<br> Please find attached invoice(s).</h3>");
