...
class WarehouseController extends CController
{
	...
	if(Yii::app()->user->name == 'Guest'):
		...
		$count = Warehouse::model()->count($criteria);
		...
		$models = Warehouse::model()->findAll($criteria);
	...
	if(Yii::app()->user->name == 'Guest'):
		...
		$model = new Warehouse();
		$data = Yii::app()->request->getPost('Warehouse');
		if($data)
		{
		$model->setAttributes($data);
		if($model->save()){

		Yii::app()->user->setFlash('saveMessage','Shop Saved Successfully....<a href="'.Yii::app()->request->baseUrl.'/warehouse/add">Add New</a> &nbsp;&nbsp;<a href="'.Yii::app()->request->baseUrl.'/warehouse">View All</a>');
		$this->refresh();
		 }
		}
	...
	if(Yii::app()->user->name == 'Guest'):
		...
		$model->setAttributes($data);
		if($model->save()){	
			Yii::app()->user->setFlash('saveMessage','Shop Modified Successfully...&nbsp;&nbsp;<a href="'.Yii::app()->request->baseUrl.'/warehouse">View All</a>');
			$this->refresh();
		   }	
		}
